...
function! s:TreeFileNode.renderToString(lineNum)
    return self._renderToString([a:lineNum], 0, 0, [], self.getChildCount() ==# 1)
endfunction
...
function! s:TreeFileNode._renderToString(lineNumL, depth, drawText, vertMap, isLastChild)
    let output = ""
    if a:drawText ==# 1
        let self.lineNum = a:lineNumL[0]
        ...
        let a:lineNumL[0] = a:lineNumL[0] + 1
    else
        let self.lineNum = a:lineNumL[0] - 1
    endif
        ...
        if len(childNodesToDraw) > 0
            ...
            if lastIndx > 0
                for i in childNodesToDraw[0:lastIndx-1]
                    let output = output . i._renderToString(a:lineNumL, a:depth + 1, 1, add(copy(a:vertMap), 1), 0)
                endfor
            endif

            "draw the last child, indicating that it IS the last
            let output = output . childNodesToDraw[lastIndx]._renderToString(a:lineNumL, a:depth + 1, 1, add(copy(a:vertMap), 0), 1)
        endif
    endif
...
function! s:createDefaultBindings()
    ...
    call NERDTreeAddKeyMap({ 'key': 'T', 'scope': "all", 'callback': s."openLatest" })
...
function! s:renderView()
    ...
    "draw the tree
    let old_o = @o
    let @o = b:NERDTreeRoot.renderToString(line("."))
    silent put o
    let @o = old_o
    ...
    lua (require "nerdtree-diagnostics").update()
endfunction
